...
export async function POST(request: Request) {
  try {
    ...
    const tempFilePath = path.join(tempDir, `upload-${Date.now()}-${Math.random().toString(36).substring(7)}.pdf`);
    ...
    const courseRegex = /^([A-Z]+)\s+([A-Z0-9]+)\s+(.+?)(?:\s+(\d+\.\d{2})(?:\s+(\d+\.\d{2}))?(?:\s+(.*))?)?$/;
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith("Course") || trimmed.startsWith("Program") || trimmed.startsWith("Level")) continue;
