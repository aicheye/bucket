    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="grid grid-cols-7 gap-px bg-base-300 border border-base-300 rounded-lg overflow-hidden">
        {FULL_DAYS.map((day) => (
          <div
            key={day}
            className="bg-base-100 p-2 text-center font-semibold text-sm"
          >
            {day.slice(0, 3)}
          </div>
        ))}
        ...
        {days.map((day, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`bg-base-100 min-h-[120px] p-2 flex flex-col gap-1 ${!isCurrentMonth ? "opacity-50" : ""}`}
            >
              <div
                className={`text-sm font-medium mb-1 w-6 h-6 flex items-center justify-center ${isToday ? "bg-primary text-primary-content rounded-full" : ""}`}
              >
                {day.getDate()}
              </div>
              <div className="flex flex-col gap-1 overflow-y-auto max-h-[100px]">
                {dayEvents.map((event) => (
                  <div
                    key={event.id}
                    className={`text-xs p-1 rounded truncate min-h-[26px] ${event.type === "deliverable" ? event.color + " text-white" : "bg-base-200 text-base-content border border-base-300"}`}
                  >
                    ...
                    <span className="font-semibold">{event.courseCode}</span>{" "}
                    {(event.title || "")
